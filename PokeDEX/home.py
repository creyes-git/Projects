...
def display_breeding(match):	
	...
	with st.container():
		...
		if egg_type_number == 2: # some Pokemon have 2 egg types
			col3.markdown('Egg Types', f'<span class="icon type-{egg_type_1.lower()}">{egg_type_1}</span>' + ', ' + f'<span class="icon type-{egg_type_2.lower()}">{egg_type_2}</span>', unsafe_allow_html=True)
		else:
			col3.markdown('Egg Types', f'<span class="icon type-{egg_type_1.lower()}">{egg_type_1}</span>', unsafe_allow_html=True)
		if percentage_male != '':	
