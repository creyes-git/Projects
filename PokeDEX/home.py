	...
	for row in similar_pokemons_df.iterrows():
		...
		try:
			with st.container():
				...
				fig = px.bar(row[1], y = row[1].index, x = name, template = "plotly_dark", range_color=[0, 250], color_discrete_sequence=px.colors.qualitative.D3, barmode="group", opacity=0.8)
