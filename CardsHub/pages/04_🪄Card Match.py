...
if submit:
    ...
    if goal == "Getting cash back":
        match = cursor.execute("SELECT * FROM cards WHERE Category = 'Cash Back' or Category = 'Store'").fetchall()
    elif goal == "Earning travel rewards":
        match = cursor.execute("SELECT * FROM cards WHERE Category = 'Travel' or Category = 'Hotel'").fetchall()
    elif goal == "Paying business expenses":
        match = cursor.execute("SELECT * FROM cards WHERE Category = 'Business' or Category = 'Store'").fetchall()
    elif goal == "Building credit history":
        match = cursor.execute("SELECT * FROM cards WHERE Category = 'Secured' or Category = 'Student'").fetchall()
    else:
        match = cursor.execute("SELECT * FROM cards").fetchall()
    ...
    for i in df.values:
        with st.container():
            ...
            with c3.container():
                c3.write("**Category:**")
                c3.image(Image.open(f"images/{i[2]}.png"), width= 55, clamp=True)
    
            with c4.expander(":[**Rewards**]"):
                for i in list(str(i[3]).split("\n")):
                    c4.markdown(f"<p style='font-family: sans-serif; color: black; font-size: 13px;'>{i}</p>", unsafe_allow_html=True)
